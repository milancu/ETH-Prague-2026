...
class ENSBridge:
    ...
    def _send_tx(self, fn: Any) -> Any:
        ...
        # EIP-1559: maxFeePerGas must be >= maxPriorityFeePerGas. On Sepolia
        # gas_price is often < 1 gwei, so naive `gas_price * 2` underflows
        # below the priority tip and the RPC rejects the tx.
        latest_block = self._eth_w3.eth.get_block("latest")
        base_fee = latest_block.get("baseFeePerGas", self._eth_w3.eth.gas_price)
        priority_fee = self._eth_w3.to_wei(1, "gwei")
        max_fee = base_fee * 2 + priority_fee  # 2x base fee headroom + tip
        tx = fn.build_transaction(
            {
                "from": self._account.address,
                "nonce": self._eth_w3.eth.get_transaction_count(
